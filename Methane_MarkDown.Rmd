...
multi_param<- function(core, pdata){ #create input for function
  for(p in colnames(pdata)) { 
    ...
    result<- fetchvars(core, 2000:2100, c(RF_CH4(), CONCENTRATIONS_CO2(), RF_TOTAL(), GLOBAL_TAS())) #results for x years
  }
  result
}
```
...
Rel_FCH4graph<-  
  ggplot(Rel_FCH4long, aes(x= year, y= value, fill= parameter))+
  geom_area()+
  labs(title= "Relative Importance Radiative Forcing CH4")
