...
results<- fetchvars(core, 1745:2100, c(EMISSIONS_CH4(),                              CONCENTRATIONS_CH4(), LIFETIME_STRAT()))
...
To examine SSP245's methane perimeter sensitivity we first 
...
param<- function(core, parameter, value) {
  ...
  reset(core)
  run(core)
  result <- fetchvars(core, 1745:2100)
...
```{r}
sensitivity_emission <- run_with_param_range(core, LIFETIME_STRAT(), seq(0, 1, 0.05))
```
