...
```{r sen-graph}
cool_names<- c("Methane Concentration", "Methane Emissions")
names(cool_names)<- c("CH4_concentration", "CH4_emissions")
...
long_fdata %>%
  ...
  labs(title = expression(CH[4]~Sensitivity~Analysis~Histogram))+
  theme_minimal()
...
ggplot(sum_data, aes(year)) +
  ...
  theme(legend.position= "none")+
  scale_fill_brewer(palette="Set1")+
  labs(y= "Mean Value", x= "Year", title = expression(CH[4]~Sensitivity~Summary))+
  theme_minimal()
...
ggplot(exp_data, aes(year)) +
  ...
  facet_wrap(~variable, scales = "free_y")+
  labs( title= expression(Multiple~Parameters~vs~Natural~CH[4]~Sensitivity), y= "Mean")+
  scale_fill_brewer(palette="Set1")+
  theme_minimal()
```
