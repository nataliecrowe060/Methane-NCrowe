...
Using `summary` we can produce statistical outputs for our relative importance data.

```{r rel-summary-CO2}
stats_cO2Conc<- summary(Rel_Co2Concentration)
stats_cO2Conc
```


```{r rel-summary-RFT}
stats_RFtot<- summary(Rel_RF_tot)
stats_RFtot
```


```{r rel-summary-RFCO2}
stats_RFCO2<- summary(Rel_RF_CO2)
stats_RFCO2
```


```{r reil-summary-global}
stats_global<- summary(Rel_global_tas)
stats_global
