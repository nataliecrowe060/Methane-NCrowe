...
data<- bind_cols(pre_data, nat_data) %>%
  as_tibble()
colnames(data)<- c("PREINDUSTRIAL_CH4", "NATURAL_CH4")
```

Using a `for` loop function, we can run both parameters of the data frame- read as different columns by our code. 

```{r function_mod}
multi_param<- function(core, pdata){
  for(p in colnames(pdata)) {
    var<- do.call(p, list())
    old_value<- fetchvars(core, NA, var)
    unit<- as.character(old_value[["units"]])
    setvar(core, NA, var, pdata[p][[1]], unit)
    reset(core)
    run(core)
    result<- fetchvars(core, 2000:2100)
    result[[p]]<- pdata[p][[1]]
...
for(rownum in 1:nrow(data)){
  output[[rownum]]<- multi_param(core, data[rownum,])
}

output_data<- output %>% bind_rows()
```
