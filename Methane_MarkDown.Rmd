...
Combining the data frames with `bind.cols`.
...
Using the function we created in the previous sensitivity analysis as a base, we can add `parameter` and `value` 1 and 2 to the input. We then replicate this doubiling throuout the function. 
...
multi_param<- function(core, parameter_1, value_1, parameter_2, value_2) {
  ...
  unit_1 <- as.character(old_value[["units"]])
  unit_2 <- as.character(old_value[["units"]])
  setvar(core, NA, parameter_1, value_1, unit_1)
  setvar(core, NA, parameter_2, value_2, unit_2)
  reset(core)
  run(core)
  result <- fetchvars(core, 2000:2100)
  result[["parameter_1_value"]] <- value_1
  result[["parameter_2_value"]] <- value_2
  result
}
...
Before running our function, we create sequences for both `NATURAL_CH4` and `PREINDUSTRIAL_CH4` with standard deviations of +/-20%. We can then incorporate these sequences and parameters into the function.
...
sensitivity_multi <- run_with_multi_param_range(core, NATURAL_CH4(), nat_seq, PREINDUSTRIAL_CH4(), pre_seq)
```
