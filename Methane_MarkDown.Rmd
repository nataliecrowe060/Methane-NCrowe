...
To visualize the spread of our new values we can create a histogram. Filling each parameter with a different color allows us to incorporate all information on one graph. 
...
```
To show the differences in values between each run, we can create a bar chart grouping parameter by `run_number`. Although the values for each run stay consistent, we must use the `summarise` and `mean` functions to condence our data frame into single values before graphing. 

```{r bar_params}
mean_fdata<- full_data %>%
  group_by(run_number) %>%
  summarise(meanNatM_TgCH4= mean(NATURAL_CH4), meanPreM_ppbvCH4= mean(PREINDUSTRIAL_CH4), meanSoilM_Years= mean(LIFETIME_SOIL), meanStratM_Years= mean(LIFETIME_STRAT))
...
... A better graph for our runs is `geom_line`, which graphs the mean of our outputs each year, as well as the maximum and minimum values. First we find the mean, standard deviation, minimum and maximum using `summarize` - then we can plot the results.

```{r summary-graph}

sum_data<-  long_fdata%>%
  group_by(variable, year) %>%
  summarize(Mean= mean(value), SD= sd(value), Minval= min(value), Maxval= max(value))

ggplot(sum_data, aes(year)) +
   geom_ribbon(aes(ymin= Minval, ymax= Maxval), fill= "pink") +
  geom_line(aes(y= Mean, color= variable), size= 0.5) +
  facet_wrap(~variable, scales = "free_y")
shutdown(core)
