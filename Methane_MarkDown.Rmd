...
To examine SSP245's Natural methane  sensitivity we first ask hector to fetch the data. We then create a function to run our parameter set to a particular value. In this case we set our value at `358.05`- twenty percent higher than the original value.

```{r call-for-param}
core <- newcore(ini_file)
run(core)
paramr<- fetchvars(core, NA, NATURAL_CH4())
```
...
Then we tell hector to run a range of values. In this case we ask for 20% above and bellow the set value of `NATURAL_CH4`(341Tg) at intervals of 5. 
...
val<- paramr %>%
  filter(value)
firstValue <- val * (1 - 0.2)
lastValue <- val * (1 + 0.2)
sensitivity_emission <- run_with_param_range(core, NATURAL_CH4(), seq(firstValue, lastValue, 5))
```

Once we have our differing parameter values we can create a graph. Using the graph function `facet_wrap`to separate the results based on variable. 
...
ggplot(sensitivity_emission) +
  ...
  labs(color = "NATURAL CH4") +
  scale_color_viridis_c() 
```

##Adding a Histogram 

Using the same `Natural_CH4` data, we can also make a histogram plot. First we use `rnorm` to set a `mean=` to the constant( 341Tg ) and a `sd` of the constant multiplied by `0.2`.

```{r}
x <- 341*0.2
data<- data.frame(value= rnorm(1000, 341, x))
```
