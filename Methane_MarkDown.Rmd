...
param<- function(core, parameter, value) {
  ...
  setvar(core, NA, parameter, 358.05, unit)
  reset(core)
  run(core)
...
Then we tell hector to runv a range of values.
...
sensitivity_emission <- run_with_param_range(core, NATURAL_CH4(), seq(0, 1, 0.05))
```

Once we have our differing parameter values we can create a graph.
...
ggplot(sensitivity_emission) +
  ...
  guides(color = guide_colorbar(title = expression(NATURAL_CH4()))) +
