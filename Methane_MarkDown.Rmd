...
Visualizing this data with stacked area graphs can make it more apparent the trend of relative importance of each variable.

```{r rel-graphs, echo= FALSE}
#Co2 Concentration
Rel_Co2Concentrationlong<- Rel_Co2Concentration %>%
  pivot_longer(!c(year, variable), names_to = "parameter", values_to = "value")

Rel_cconc_graph<-
  ggplot(Rel_Co2Concentrationlong, aes(x= year, y= value, fill= parameter))+
  geom_area()+
  labs(title= "Relative Importance Co2 Concentration")
...
Rel_RF_graph<- 
  ggplot(Rel_RF_totlong, aes(x= year, y= value, fill= parameter))+
  geom_area()+
  labs(title= "Relative Importance RF Total")
...
Rel_RF_CO2graph<-  
  ggplot(Rel_RF_CO2long, aes(x= year, y= value, fill= parameter))+
  geom_area()+
  labs(title= "Relative Importance RF CO2")

#Global Tas
Rel_global_taslong<- Rel_global_tas %>%
  pivot_longer(!c(year, variable), names_to = "parameter", values_to = "value")

Rel_global_tasgraph<-  ggplot(Rel_global_taslong, aes(x= year, y= value, fill= parameter))+
  geom_area()+
  labs(title= "Relative Importance Global Tas")
...
CO2 concentration is influenced most by `PREINDUSTRIAL_CH4` with both the highest mean and highest range- meaning in every year calculated the parameter consistently was highest. This is consistant throughout the other parameters as well.

```{r rel-summary-CO2}
stats_Co2Concentration<- stats_function(Rel_Co2Concentrationlong)
stats_Co2Concentration
```
Radioactive forcing total 
...
```{r rel-summary-RFCO2}
stats_RFCO2<- stats_function(Rel_RF_CO2long)
stats_RFCO2
```
Global tas
